...
UNEXPECTED_OUTPUT="Hip-Hop - Playboi Carti - Magnolia"
if grep -q -i "$UNEXPECTED_OUTPUT" /tmp/OUTPUT; then
    printf "\033[38;5;1mError '$UNEXPECTED_OUTPUT' was found in output\033[0m\n"    
    echo ">>>>>>> Program's Output <<<<<<"
    cat /tmp/OUTPUT
    echo "Test Failed"
    exit 1
else
    echo "Did not find unexpected output."    
fi 
